...
const COUPLES_KEY = 'family-birthdays-couples';

// ── Which family this browser tab follows ───────────────────────────────────
// sessionStorage is per-tab, so switching family in one tab can't silently
// hijack another tab on reload. A brand-new tab has no sessionStorage entry
// yet, so it falls back to localStorage (the last family used anywhere in
// this browser) to keep "remember my family" working across restarts.
function resolveFamilyCode() {
  try {
    const fromSession = sessionStorage.getItem(CODE_KEY);
    if (fromSession) return fromSession;
    const fromLocal = localStorage.getItem(CODE_KEY) || '';
    if (fromLocal) sessionStorage.setItem(CODE_KEY, fromLocal);
    return fromLocal;
  } catch { return ''; }
}
function persistFamilyCode(code) {
  try {
...
// ── Local cache, namespaced per family code ─────────────────────────────────
// Without a code (local-only mode) these behave exactly as before. With a
// code, each family gets its own cache bucket so two tabs on two different
// families never overwrite each other's local copy. Falls back to the
// pre-namespacing flat key once, so existing users don't lose their cache.
function familyCacheGet(base, code) {
  try {
    if (code) {
      const v = localStorage.getItem(`${base}:${code}`);
      return v !== null ? v : localStorage.getItem(base);
    }
    return localStorage.getItem(base);
  } catch { return null; }
}
function familyCacheSet(base, code, value) {
...
export default function App() {
  const [members, setMembers] = useState(() => {
    try {
      const code = WORKER_URL ? resolveFamilyCode() : '';
      if (localStorage.getItem(VERSION_KEY) !== STORAGE_VERSION) {
        localStorage.setItem(VERSION_KEY, STORAGE_VERSION);
        localStorage.removeItem(STORAGE_KEY);
        if (code) localStorage.removeItem(`${STORAGE_KEY}:${code}`);
        return DEFAULT_MEMBERS;
      }
      const s = familyCacheGet(STORAGE_KEY, code);
      if (s) return sanitizeMembers(JSON.parse(s));
    } catch {}
    return DEFAULT_MEMBERS;
  });
  const [nextId,       setNextId]       = useState(()=>Math.max(0, ...members.map(m=>m.id))+1);
  const [coupleDates,  setCoupleDates]  = useState(()=>{
    try {
      const s = familyCacheGet(COUPLES_KEY, WORKER_URL ? resolveFamilyCode() : '');
  ...
  const [familyCode, setFamilyCodeRaw] = useState(() =>
    WORKER_URL ? resolveFamilyCode() : ''
  );
  const [showSetup,  setShowSetup]  = useState(() =>
    !!WORKER_URL && !resolveFamilyCode()
  );
  const [familyName, setFamilyName] = useState(() =>
    familyCacheGet(NAME_KEY, WORKER_URL ? resolveFamilyCode() : '') || ''
  );
  ...
  useEffect(()=>{ try { familyCacheSet(STORAGE_KEY, familyCode, JSON.stringify(members)); } catch {} },[members, familyCode]);
  useEffect(()=>{ try { familyCacheSet(COUPLES_KEY, familyCode, JSON.stringify(coupleDates)); } catch {} },[coupleDates, familyCode]);
  useEffect(()=>{ try { familyCacheSet(NAME_KEY, familyCode, familyName); } catch {} },[familyName, familyCode]);
